...
task("view_db", function()
    ...
    on_run(function()
        ...
        assert(_db_files, "[view_db] db_file is required, use `--db_file` to specify it")
        ...
        for _, db_file in ipairs(db_files) do
            ...
            for _, t in ipairs(tables) do
                local result
                if is_duckdb then
                    result = os.iorun([[duckdb %s -header -list -c "SELECT * FROM '%s' WHERE address like '%s';"]],
                        db_file, t, addr)
                else
                    result = os.iorun([[sqlite3 -header "%s" "SELECT * FROM '%s' WHERE address like '%s';"]], db_file, t,
                        addr)
                end
