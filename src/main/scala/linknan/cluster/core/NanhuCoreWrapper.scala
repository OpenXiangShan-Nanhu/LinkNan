...
class NanhuV5CoreConfig extends Config((site, here, up) => {
  case XLen => 64
  case XSCoreParamsKey => XSCoreParameters()
  case TLUserKey => TLUserParams(aliasBits = site(DcacheKey).aliasBitsOpt.getOrElse(0), vaddrBits = 44) // 50 - 6
  case PMParameKey => PMParameters()
  case PerfCounterOptionsKey => PerfCounterOptions(!site(DebugOptionsKey).FPGAPlatform, false, 0)
  case LogUtilsOptionsKey => LogUtilsOptions(site(DebugOptionsKey).EnableDebug, site(DebugOptionsKey).EnablePerfDebug, site(DebugOptionsKey).FPGAPlatform)
})
